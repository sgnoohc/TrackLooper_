...
{
    ana.histograms.addVecHistogram(TString::Format("Segment_dz"),400,-20,20,[&](){return dzValues;});
    ...
    ana.histograms.addVecHistogram(TString::Format("Segment_barrel_dz"),400,-20,20,[&](){return barreldzValues;});
    ...
    ana.histograms.addVecHistogram(TString::Format("Segment_endcap_drt"),400,-20,20,[&](){return endcapdrtValues;});
    ...
    {
        ana.histograms.addVecHistogram(TString::Format("Segment_dz_layer_%ld",i+1),200,-20,20,[&,i](){return layerdzValues[i];});
        ...
        ana.histograms.addVecHistogram(TString::Format("Segment_barrel_dz_layer_%ld",i+1),400,-20,20,[&,i](){return layerBarreldzValues[i];});
        ...
        if(i < 5)
        {
            ana.histograms.addVecHistogram(TString::Format("Segment_endcap_drt_layer_%ld",i+1),400,-20,20,[&,i](){return layerEndcapdrtValues[i];});
    ...
    {
        ana.histograms.addVecHistogram(TString::Format("Segment_endcap_drt_ring_%ld",i+1),400,-20,20,[&,i](){return ringEndcapdrtValues[i];});
...
void StudySegmentCuts::doStudy(SDL::Event &event, std::vector<std::tuple<unsigned int, SDL::Event*>> simtrkevents)
{
    ...
    for(auto &module:moduleList)
    {
        ...
        for(auto &sg:segments)
        {
            ...
            float dAlphaInnerMDSegment = dPhiChangeInnerMD - dPhiChangeSegemnt;
            ...
            dAlphOuterMDSegmentValues.push_back(dAlphaOuterMDSegment);
            ...
            if(module->subdet() == SDL::Module::Barrel)
            {
                ...
                barrelInnerMDSegmentValues.push_back(dAlphaInnerMDSegment);
                barrelOuterMDSegmentValues.push_back(dAlphaOuterMDSegment);
                barrelInnerMDOuterMDValues.push_back(dAlphaInnerMDOuterMD);
