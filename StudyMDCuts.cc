...
void StudyMDCuts::bookStudy()
{
    ...
    {
        ana.histograms.addVecHistogram(TString::Format("MD_dz_layer_%d",i),400,-20,20,[&,i](){return layerdzValues[i];});
        ana.histograms.addVecHistogram(TString::Format("MD_dPhi_layer_%d",i),200,-6.28,6.28,[&,i](){return layerdPhiValues[i];});
        ana.histograms.addVecHistogram(TString::Format("MD_dPhiChange_layer_%d",i),200,-6.28,6.28,[&,i](){return layerdPhiChangeValues[i];});


        ana.histograms.addVecHistogram(TString::Format("MD_barrel_dz_layer_%d",i),400,-20,20,[&,i](){return layerBarreldzValues[i];});
        ana.histograms.addVecHistogram(TString::Format("MD_barrel_dPhi_layer_%d",i),200,-6.28,6.28,[&,i](){return layerBarreldPhiValues[i];});
        ana.histograms.addVecHistogram(TString::Format("MD_barrel_dPhiChange_layer_%d",i),200,-6.28,6.28,[&,i](){return layerBarreldPhiChangeValues[i];});

        if(i < 6)
        {

            ana.histograms.addVecHistogram(TString::Format("MD_endcap_dz_layer_%d",i),400,-20,20,[&,i](){return layerEndcapdzValues[i];});
            ana.histograms.addVecHistogram(TString::Format("MD_endcap_dPhi_layer_%d",i),200,-6.28,6.28,[&,i](){return layerEndacpdPhiValues[i];});
